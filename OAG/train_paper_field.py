...
def node_classification_sample(seed, pairs, time_range, cand_list):
    '''
...
def prepare_data(pool, sel_train_pairs, train_range, sel_valid_pairs, valid_range, cand_list):
    '''
    ...
    for batch_id in np.arange(args.n_batch):
        p = pool.apply_async(node_classification_sample, args=(randint(), \
            sel_train_pairs, train_range, cand_list))
        jobs.append(p)
    p = pool.apply_async(node_classification_sample, args=(randint(), \
            sel_valid_pairs, valid_range, cand_list))
    jobs.append(p)
    return jobs
...
parser.add_argument('--n_pool', type=int, default=1,
                    help='Number of process to sample subgraph')
parser.add_argument('--n_batch', type=int, default=1,  # 32
...
if __name__ == '__main__':
    ...
    st = time.time()
    jobs = prepare_data(pool, sel_train_pairs, train_range, sel_valid_pairs, valid_range, cand_list)
    ...
    with torch.no_grad():
        ...
        for _ in range(10):
            node_feature, node_type, edge_time, edge_index, edge_type, x_ids, ylabel = \
                node_classification_sample(randint(), test_pairs, test_range, cand_list)
